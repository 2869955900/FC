...
leftatrialdiam = st.sidebar.number_input("Left Atrial Diameter (LeftAtrialDiam):", min_value=0.0, max_value=100.0, value=5.5, step=0.1)  # 左房直径输入框
...
if st.button("Make Prediction"):  # 如果点击了预测按钮
    ...
    for i, v in enumerate([sample_prob['No Electrical Cardioversion'], sample_prob['Needs Electrical Cardioversion']]):  # 为每个条形图添加概率文本标签
        plt.text(v + 0.0001, i, f"{v:.3f}", va='center', fontsize=14, color='black', fontweight='bold')  # 设置标签位置、字体加粗
